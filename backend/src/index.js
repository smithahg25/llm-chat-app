...
const llm_1 = require("./sdk/llm");
const logger_1 = require("./logger");
const REQUIRED_ENV_VARS = ['DATABASE_URL', 'OPENAI_API_KEY', 'GEMINI_API_KEY'];
for (const envVar of REQUIRED_ENV_VARS) {
    if (!process.env[envVar]) {
        logger_1.logger.fatal(`Missing required environment variable: ${envVar}`);
        process.exit(1);
    }
}
...
app.post('/ingest', async (req, res) => {
    try {
        ...
        logger_1.logger.error({ error }, 'Error in /ingest');
...
app.post('/chat', async (req, res) => {
    ...
    try {
        ...
        const aiResponse = await llm_1.InstrumentLLM.generateChat({
            provider, model,
        ...
        logger_1.logger.error({ error }, 'Chat error');
...
app.post('/chat/stream', async (req, res) => {
    ...
    try {
        ...
        logger_1.logger.error({ error }, 'Chat stream error');
